...
logging.basicConfig(filename=get_log_file_path(), level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def request_singular(data_inicio, data_fim, jurisprudencia_procurada, tribunais_selecionados):
    ...
    if tribunais_selecionados.lower() == "todos":
        ...
        tribunais_para_pesquisar = [t for t in tribunais_disponiveis['lista_tribunais'] if t != "TODOS"]
    else:
        # Caso contrário, usar os tribunais fornecidos pelo usuário
        tribunais_config_path = os.path.join(os.path.dirname(__file__), 'config', 'tribunais.json')
        tribunais_disponiveis = load_tribunais(tribunais_config_path)
        tribunais_para_pesquisar = [t.strip() for t in tribunais_selecionados.split(',') if t.strip() in tribunais_disponiveis['lista_tribunais']]
            ...
            if response.status_code == 200:
                try:
                    ...
                    for item in resultado_atual['items']:
                        ...
                        index.upsert(vectors=[{"id": vector_id, "values": dummy_vector, "metadata": item}])
                        ...
                        if texto_para_categorizar:
                            ...
                            if openai_api_key:
                                resultado_categorizacao = processar_com_ia(texto_para_categorizar, openai_api_key)
...
def load_tribunais(file_path):
    """Carrega a lista de tribunais do arquivo JSON."""
    TRIBUNAIS_FILE = os.path.join(os.path.dirname(__file__), "tribunais.json")
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
        logging.error(f"Erro: Arquivo de tribunais não encontrado em {file_path}")
        sys.exit(1)
    except json.JSONDecodeError:
        logging.error(f"Erro: Não foi possível decodificar o arquivo JSON de tribunais em {file_path}")
        sys.exit(1)

def load_categorias(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                categorias[row['categoria_id']] = row['nome_categoria']
        return categorias
    except FileNotFoundError:
        logging.error(f"Erro: Arquivo de categorias não encontrado em {file_path}")
        sys.exit(1)
    except Exception as e:
        logging.error(f"Erro ao carregar categorias do arquivo {file_path}: {e}")
        sys.exit(1)

def gerar_descricao_com_ia(texto, api_key):
    """Processa o texto usando a API da OpenAI para gerar uma descrição."""
    pasta_configs = os.path.join(os.path.dirname(__file__), 'config')
